...
## 2026-09-14 本地待验收：右键菜单、全局休假与 09:00 开放

- 单元格右键菜单固定为“新增排班 / 休假（已休假时为取消休假）/ 查看历史记录”。新增排班
  会在原单元格追加独立块；总览空白格先选择小组，右键已有块则沿用该块所属小组。
- `timeOff[week][personId_date]` 是跨小组共享的人员日期状态。休假时总览与所有小组都只显示
  一个灰色休假块，原有排班仍保存在 `schedules` 中并隐藏，取消休假后原样恢复。
- 休假格禁止新增、粘贴、填充、拖入、Delete 清空和撤销/重做恢复排班；设置和取消休假都进入
  单元格历史。历史抽屉不再用当前小组二次过滤跨组事件，独立块追加按“新增”记录。
- 普通账号的下一排班周改为北京时间周一 09:00 开放；张雅镜、简慧仪、林俊凯仍不受未来周
...
## 2026-09-20 前台术语统一为“请假”

- 网页提示、右键菜单、单元格、操作反馈、图片导出和修改记录统一显示“请假/取消请假”。
- 已有请假状态无需迁移，会直接按新文案显示；既有历史中存储的“休假”也会在展示层转换为“请假”。
- 内部字段 `timeOff`、历史动作 `leaveSet/leaveClear` 及旧历史原始值保持不变，避免改写生产数据。

## 2026-09-24 排班开放时间调整为周一 14:00

- 普通账号填写下一排班周期的开放时间改为北京时间周一下午 2:00；14:00 前继续提示尚未进入排班时间。
- 前端预判、同步冲突提示和服务端权限边界统一使用 14:00，三位受保护管理员仍不受未来周期限制。
- 边界测试覆盖周一 13:59:59 仍关闭、14:00:00 准时开放；不新增或迁移任何业务数据字段。
