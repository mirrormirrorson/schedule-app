...
- 日期表头按工作日、休息日整块着色并显示“班/休”；2026 年国务院调休日期内置在
  ...
  不增加 Neon 查询或网络流量。
- 普通工作日使用工作日底色，周末及法定休息日使用休息日底色，调休上班日使用单独底色；
  小组表与总览表共用同一套日历判断。
- 节假日表按年份维护；下一年度国务院办公厅发布正式安排后，需要在
  `public/js/work-calendar.js` 增补对应年度数据。未配置的年份按普通周一至周五/周末显示。
- 本次发布不包含全局休假功能，也不新增任何排班业务数据字段。
