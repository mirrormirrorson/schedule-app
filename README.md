...
## 2026-09-07 真实日历与全局休假（本地待验收）

- 日期表头按工作日、休息日整块着色并显示“班/休”；2026 年国务院调休日期内置在
  `public/js/work-calendar.js`，例如 9 月 20 日虽为周日仍显示“班”。运行时不调用第三方接口。
- 新增 `timeOff[week][personId_date]` 稀疏状态：一次记录人员某天的全局休假，总览只显示一条，
  所有小组共读同一条，不复制进各组 `schedules`。
- 总览格右上角“休”由张雅镜、简慧仪、林俊凯设置/取消，可填可选备注；服务端同时校验管理员
  身份。修改进入现有保存队列、冲突检测、历史和备份。
- 已有排班不会因设置休假被自动删除，而是显示冲突；新增编辑、粘贴、填充和拖动会跳过或阻止
  休假格，避免继续误排。
- 本地分支 `agent/calendar-global-leave`，隔离 FileStore 位于被忽略的
  `tmp-calendar-leave-preview/db.json`，预览 `http://127.0.0.1:3471/`。样例只存在隔离文件：
  肖秋金 2026-09-20“调休示例”。52/52 自动测试通过，尚未推送或部署。
